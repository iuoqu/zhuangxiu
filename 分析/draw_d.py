...
def draw():
    ...
    # 南区纵向支通道
    aisle_band(s, *D.SPUR)
    s.txt((D.SPUR[0]+D.SPUR[2])/2, 17000, '纵向支通道 1400', 290, C['keeptx'], weight='500', rot=-90)
    # ②轴柱
    ...
    for (rx0, ry0, rx1, ry1, name, cap, door) in D.ROOMS:
        ...
        sz = 360 if big else 300
        s.multi((rx0+rx1)/2, (ry0+ry1)/2 - (200 if big else 260), [name, f'{a:.1f} ㎡', cap],
                sz, txt, weight='600')
        # 门位示意
    ...
    py = N_ZONE[1] + HEAD/2
    px = [(600, 20500), (600, 19674), (ax, 19674), (ax, SPINE_Y[0]+900), ((cx[0]+cx[1])/2, SPINE_Y[0]+900),
    ...
    s.circ(600, 20500, 220, C['demo'])
    s.txt(950, 20830, '最不利房间', 290, C['demo'], anchor='start', weight='600')
    ...
    s.txt(-3200, -3300,
          ...
          f'{nf.seats} 工位 ＋ 大会议室 38.4㎡ ＋ 中会议室 20.9㎡ ＋ 洽谈间 ×4 ＋ 茶水区 14.3㎡',
    ...
    notes = [
        ...
        '＋ 南区主通道 1800 ＋ 南区纵向支通道 1400。大、中会议室直接开门到南区主通道，'
        '四间洽谈与茶水区开门到纵向支通道，不再另设走道。',
        '3. 疏散：北区开敞办公区最不利点（西北角）至办公区疏散门直线距离 ≈ 19.9 m；'
        '最不利房间（西南角洽谈间 A）室内任一点至本房间门 ≤ 3.8 m、房门至疏散门 ≈ 22.9 m。'
        '两项均明显低于喷淋保护下限值 27.5 m，也比方案 B 的 25.2 m 宽松 —— '
        '因为原来最深的西南角现在是有独立疏散门的房间，而不是开敞办公区。仍须由消防设计单位复核防烟分区与排烟方式。',
        '4. 茶水区紧贴保留的备餐间 / 茶水间（既有给排水点位），可就近接水，避免长距离排水；'
        '打通 X=12751 处一道墙即可与保留茶水间 23 ㎡ 合并为约 37 ㎡ 的完整茶水区，并占据南向采光段。',
        '5. 第 3 条工位带在东端让出 2 列（3.0 × 2.5 m）作打印 / 储物 / 电话亭 ×2，'
        f'这也是本方案工位数落在 {nf.seats} 而不是 54 的原因。',
        '6. 大会议室 5.9 × 6.5 m 可放 4.8 × 1.6 m 会议桌（18–20 人）；中会议室 5.5 × 3.8 m 放 2.4 × 1.2 m 桌（8–10 人）；'
